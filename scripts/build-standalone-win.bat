...
node -e "require('fs').copyFileSync(process.execPath, process.argv[1])" "!EXE_TARGET!"
if %ERRORLEVEL% neq 0 (
...
if defined SIGNTOOL (
    ...
    "%SIGNTOOL%" remove /s "!EXE_TARGET!" >nul 2>nul
    if %ERRORLEVEL% equ 0 (
...
npx --yes postject "!EXE_TARGET!" NODE_SEA_BLOB build\sea-prep.blob --sentinel-fuse NODE_SEA_FUSE_fce680ab2cc467b6e072b8b5df1996b2
if %ERRORLEVEL% neq 0 (
...
REM Get current year for copyright
for /f "usebackq tokens=*" %%i in (`powershell -Command "Get-Date -Format yyyy"`) do set "YEAR=%%i"

npx rcedit "!EXE_TARGET!" --set-version-string "CompanyName" "Todis Consulting" --set-version-string "FileDescription" "Library to translate KSEF XML files into PDF format" --set-version-string "ProductName" "KSEF PDF Generator" --set-version-string "LegalCopyright" "Copyright (C) !YEAR! Todis Consulting" --set-file-version "!APP_VERSION!.0" --set-product-version "!APP_VERSION!.0"
if %ERRORLEVEL% neq 0 (
...
echo ========================================
echo.
echo Standalone executable created: !EXE_TARGET!
echo.

REM Get file size
for %%A in ("!EXE_TARGET!") do set SIZE=%%~zA
...
if %ERRORLEVEL% equ 0 (
    ...
    if !ERRORLEVEL! equ 1 (
        ...
        if !ERRORLEVEL! equ 0 (
            ...
            for %%A in ("!EXE_TARGET!") do set NEW_SIZE=%%~zA
...
echo.
echo You can now copy !EXE_TARGET! to any Windows machine
echo without requiring Node.js installation!
echo.
echo Usage:
echo   !EXE_TARGET! -i input.xml -o output.pdf -t invoice
echo.
pause

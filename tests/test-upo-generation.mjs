#!/usr/bin/env node
// Test: UPO PDF generation from XML
import { execSync } from 'child_process';
import { existsSync, unlinkSync } from 'fs';

const TEST_NAME = 'UPO PDF Generation';
const EXECUTABLE = process.platform === 'win32' 
  ? 'bin\\ksef-pdf-generator.exe' 
  : 'bin/ksef-pdf-generator.sh';
const INPUT_FILE = 'assets/upo.xml';
const OUTPUT_FILE = 'tests/test-upo-output.pdf';
...
try {
  execSync(`${EXECUTABLE} -i "${INPUT_FILE}" -o "${OUTPUT_FILE}" -t upo`, { stdio: 'pipe' });

...
Newly Applied
- Version bump to `1.0.0` in `package.json` and `package-lock.json` while keeping fork-specific metadata and scripts.
...
- `package.json`, `package-lock.json`: preserved fork package identity and scripts; applied upstream version bump only.
